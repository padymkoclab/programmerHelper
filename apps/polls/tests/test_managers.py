...
class VoteManagerTest(TestCase):
    ...
    def test_get_statistics_count_votes_by_months_for_past_year_if_votes_added_tonow(self):
    ...
    def test_get_statistics_count_votes_by_months_for_past_year_if_votes_added_in_diffrent_time_for_past_year(self):
        ...
        Vote.objects.filter(pk=vote.pk).update(date_voting=self.now - relativedelta(months=2))

        # three months ago
        vote = self.user1.votes.create(poll=self.poll1, choice=self.poll1.choices.first())
        Vote.objects.filter(pk=vote.pk).update(date_voting=self.now - relativedelta(months=3))

        # four months ago
        vote = self.user2.votes.create(poll=self.poll1, choice=self.poll1.choices.first())
        Vote.objects.filter(pk=vote.pk).update(date_voting=self.now - relativedelta(months=4))
        ...
        Vote.objects.filter(pk=vote.pk).update(date_voting=self.now - relativedelta(months=6))
        ...
        Vote.objects.filter(pk=vote.pk).update(date_voting=self.now - relativedelta(months=8))

        # nine months ago
        vote = self.user1.votes.create(poll=self.poll3, choice=self.poll3.choices.first())
        Vote.objects.filter(pk=vote.pk).update(date_voting=self.now - relativedelta(months=9))
        ...
        # eleven months ago
        vote = self.user3.votes.create(poll=self.poll3, choice=self.poll3.choices.first())
        Vote.objects.filter(pk=vote.pk).update(date_voting=self.now - relativedelta(months=11))

        self.assertListEqual(
            Vote.objects.get_statistics_count_votes_by_months_for_past_year(),
            [
                ((self.now - relativedelta(months=11)).strftime('%b %Y'), 1),
                ((self.now - relativedelta(months=10)).strftime('%b %Y'), 1),
                ((self.now - relativedelta(months=9)).strftime('%b %Y'), 1),
                ((self.now - relativedelta(months=8)).strftime('%b %Y'), 1),
                ((self.now - relativedelta(months=7)).strftime('%b %Y'), 1),
                ((self.now - relativedelta(months=6)).strftime('%b %Y'), 1),
                ((self.now - relativedelta(months=5)).strftime('%b %Y'), 1),
                ((self.now - relativedelta(months=4)).strftime('%b %Y'), 1),
                ((self.now - relativedelta(months=3)).strftime('%b %Y'), 1),
                ((self.now - relativedelta(months=2)).strftime('%b %Y'), 1),
                ((self.now - relativedelta(months=1)).strftime('%b %Y'), 1),
                (self.now.strftime('%b %Y'), 1),
            ],
        )
    ...
    def test_get_statistics_count_votes_by_months_for_past_year_with_mock_now_datetime(self):
