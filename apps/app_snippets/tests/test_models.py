...
class BaseTestClass_for_prepopulated_data(TestCase):
    ...
    def setUpTestData(cls):
        ...
        self.assertEqual(snippet.author, data['author'])
        ...
        for i in range(3):
            Factory_CommentGeneric(content_object=snippet)
            Factory_OpinionGeneric(content_object=snippet)
...
def signal_for_keeping_old_account(sender, instance, **kwargs):
    ...
    if sender in MODELS_WITH_FK_ACCOUNT:
        ...
        self.snippet.author = data_for_update['author']
        ...
        self.assertEqual(self.snippet.author, data_for_update['author'])
    ...
    def test_get_scope_of_the_snippet(self):
        ...
        random_accounts = Account.objects.exclude(pk=self.snippet.author.pk).random_accounts(10)
        ...
        for couple in zip(random_accounts, choices_for_is_useful):
            self.snippet.opinions.create(user=couple[0], is_useful=couple[1])
