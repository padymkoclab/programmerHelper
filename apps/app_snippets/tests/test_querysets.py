...
class Test_SnippetQuerySet(TestCase):
    ...
    def setUpTestData(cls):
        ...
        # add opinions
        accounts = Account.objects.exclude(pk__in=Snippet.objects.values('author'))
        ...
        for couple in zip(accounts, opinions_for_snippet1):
            cls.snippet1.opinions.create(user=couple[0], is_useful=couple[1])
        for couple in zip(accounts, opinions_for_snippet2):
            cls.snippet2.opinions.create(user=couple[0], is_useful=couple[1])
        for couple in zip(accounts, opinions_for_snippet3):
            cls.snippet3.opinions.create(user=couple[0], is_useful=couple[1])
        for couple in zip(accounts, opinions_for_snippet4):
            cls.snippet4.opinions.create(user=couple[0], is_useful=couple[1])
        for couple in zip(accounts, opinions_for_snippet5):
            cls.snippet5.opinions.create(user=couple[0], is_useful=couple[1])
        # add comments
        cls.snippet1.comments.create(author=Account.objects.random_accounts(1), text_comment='Hi, it is very nice snippet.')
        cls.snippet1.comments.create(author=Account.objects.random_accounts(1), text_comment='Bad snippet.')
        cls.snippet1.comments.create(author=Account.objects.random_accounts(1), text_comment='Interesting snippet.')
        cls.snippet2.comments.create(author=Account.objects.random_accounts(1), text_comment='Nice snippet.')
        cls.snippet2.comments.create(author=Account.objects.random_accounts(1), text_comment='This is charm.')
        cls.snippet2.comments.create(author=Account.objects.random_accounts(1), text_comment='No. This is awesome snipet.')
        cls.snippet2.comments.create(author=Account.objects.random_accounts(1), text_comment='Thank you author`s of the snippet.')
        cls.snippet3.comments.create(author=Account.objects.random_accounts(1), text_comment='Simple snippet.')
        cls.snippet4.comments.create(author=Account.objects.random_accounts(1), text_comment='Stupid snippet.')
        cls.snippet4.comments.create(author=Account.objects.random_accounts(1), text_comment='Amazing snippet.')
        cls.snippet5.comments.create(author=Account.objects.random_accounts(1), text_comment='Non insteresting snippet.')
    ...
    def test_snippets_with_count_tags_opinions_comments_scopes_and_count_good_or_bad_opinions(self):
        ...
        # Snippet.objects.snippets_with_count_opinions().snippets_with_count_good_opinions().snippets_with_count_bad_opinions().snippets_with_count_comments().first().count_good_opinions
