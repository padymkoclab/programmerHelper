...
from mylabour.factories import Factory_OpinionUserModel, Factory_Comment
...
OpinionAboutBook.objects.filter().delete()
...
class Factory_OpinionAboutBook(Factory_OpinionUserModel):

    class Meta:
        model = OpinionAboutBook


class Factory_BookComment(Factory_Comment):
...
for i in range(30):
    ...
    random_users_for_opinions = random.sample(tuple(Accounts), random.randrange(0, min_high_limiter))
    random_users_for_comments = random.sample(tuple(Accounts), random.randrange(0, min_high_limiter))
    for user in random_users_for_opinions:
        Factory_OpinionAboutBook(book=book, user=user)
    for author in random_users_for_comments:
        Factory_BookComment(book=book, author=author)
