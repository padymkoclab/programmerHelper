...
class Test_ViewPrivacyPolicy(TestCase):
    """
    Testing view ViewPrivacyPolicy of application "privacy_policy"
    """

    def setUp(self):
        factory = RequestFactory()
        url = reverse('privacy_policy:privacy_policy')
        self.request = factory.get(url)

    def test_render_view(self):
        response = ViewPrivacyPolicy.as_view()(self.request)
        self.assertEqual(response.status_code, 200)
        self.assertIn('privacy_policy/privacy_policy.html', response.template_name)
